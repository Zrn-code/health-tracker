...
class TestDailyDataAPI:
    ...
    def setup_authenticated_user(self, api_client):
        ...
        if register_response.status_code not in [200, 201] or register_response.status_code == 503:
            return None
    ...
    def test_submit_daily_data_missing_fields(self, api_client, server_available):
        """測試提交缺少必要欄位的日常數據"""
    ...
    def test_submit_daily_data_success(self, api_client, server_available):
        """測試提交日常數據"""
        if not server_available:
            pytest.skip("服務器不可用，跳過測試")
    ...
    def test_get_daily_data_success(self, api_client, server_available):
        """測試獲取日常數據"""
        if not server_available:
            pytest.skip("服務器不可用，跳過測試")
    ...
    def test_get_daily_data_unauthorized(self, api_client, server_available):
        """測試未授權訪問日常數據"""
        if not server_available:
            pytest.skip("服務器不可用，跳過測試")
    ...
    def test_get_health_suggestion(self, api_client, server_available):
        """測試獲取健康建議"""
        if not server_available:
            pytest.skip("服務器不可用，跳過測試")
